...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            // Stats & Core Settings
            const [stdSnap, compSnap, jobSnap, appSnap, revSnap, chatSnap, reportsSnap, settingsSnap, hubSnap] = await Promise.all([
                ...
                getDoc(doc(db, "settings", "forbiddenContent")),
                getDoc(doc(db, "settings", "stajhub"))
            ]);
            ...
            if (settingsSnap.exists()) setForbidden(settingsSnap.data());
            if (hubSnap.exists()) setSystemSettings(hubSnap.data());

            // Data Lists
            setChats(chatSnap.docs.map(d => ({ id: d.id, ...d.data() })));
            setReports(reportsSnap.docs.map(d => ({ id: d.id, ...d.data() })));
            setJobs(jobSnap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0)));
            setReviews(revSnap.docs.map(d => ({ id: d.id, ...d.data() })));

            // User list (Limited for performance)
            ...
            setError("Sistem verileri senkronize edilemedi.");
        }
        setLoading(false);
    };
