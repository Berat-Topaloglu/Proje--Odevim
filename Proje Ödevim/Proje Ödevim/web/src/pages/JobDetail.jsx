...
export default function JobDetail() {
    ...
    const handleApply = async () => {
        ...
        // --- DİNAMİK ZORUNLU ALAN KONTROLÜ (SENKRONİZE) ---
        if (job.cvRequired && !cvFile) {
            setModalError("Lütfen Özgeçmiş (CV) dosyanızı seçin.");
            showNotification("Bu ilan için CV yüklemeniz zorunludur.", "error", "Eksik Dosya");
            setApplying(false);
            return;
        }

        if (job.portfolioRequired && (!portfolioUrl || portfolioUrl.trim() === "")) {
            setModalError("Lütfen Portfolyo / LinkedIn / Repo linkinizi girin.");
            showNotification("Bu ilan için portfolyo veya repo linki zorunludur.", "error", "Eksik Bilgi");
            setApplying(false);
            return;
        }
        // --------------------------------------------------
    ...
    return (
        <div className="page-wrapper">
            <div className="content-wrapper page-enter" style={{ maxWidth: 800 }}>
                ...
                {showApplyModal && (
                    <div className="modal-overlay" onClick={() => { setShowApplyModal(false); clearStatus(); }}>
                        <div className="modal-card card" onClick={(e) => e.stopPropagation()}>
                            <h2 className="modal-title">📨 Başvur: {job.title}</h2>
                            <p className="modal-subtitle">{job.companyName}</p>

                            {modalError && <div className="alert alert-error mb-16">{modalError}</div>}
